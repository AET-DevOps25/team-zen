...
export const calculateJournalStats = (journals: Array<JournalEntry>) => {
  const totalJournals = journals.length;
  const totalWords = journals.reduce(
    (sum, journal) => sum + (journal.summary?.split(' ').length || 0),
    0,
  );
  const avgMood =
    totalJournals > 0
      ? (
          journals.reduce(
            (sum, journal) => sum + getMoodValue(journal.dailyMood),
            0,
          ) / totalJournals
        ).toFixed(1)
      : '0';
  const avgWordsPerJournal =
    totalJournals > 0 ? Math.round(totalWords / totalJournals) : 0;
